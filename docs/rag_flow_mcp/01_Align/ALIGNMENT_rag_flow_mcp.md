...
## 5. 生产环境升级 (v2.1 P0 计划)
**背景**: 为了将 RAG Flow MCP 打造为团队级生产力基础设施，需解决文档修改的破坏性风险、并发冲突和质量稳定性问题。
...
2.  **轻量级副本机制 (Collaboration)**:
    ...
    *   **规范**: 所有的自动化修改（进化、填充）必须输出到同目录下 `{原文件名}- *规范*: 所有的自动化修改（进化、填充）必须输出到同目录下 `{原文件名}-ai-{timestamp}.md`，绝不触碰原文件；用户可一键对比或手动合并。
...
### 6.1 环境配置增强
*   **需求**: 增加基本的 `.env` 配置支持。
*   **目的**: 保证调试和打包后能方便地连接到 RAGFlow 服务，实现配置与代码分离。
*   **关键配置项**: `RAGFLOW_API_KEY`, `RAGFLOW_HOST_URL`。

### 6.2 知识库 CRUD 能力
*   **需求**: 增加对知识库 (Dataset) 和文档 (Document) 的基本增删改查功能。
*   **目的**: 赋予 MCP Server 直接管理知识库的能力，而不仅是消费知识。
*   **涉及操作**: List Datasets, Create Dataset, Upload Document, Delete Document 等。

### 6.3 工具架构解耦
*   **需求**: 将任务和引擎解耦为单元工具，明确划分为“逻辑工具”和“实施工具”。
*   **逻辑工具 (Logic Tools)**: 负责按照用户意图或工作流推进任务（如：填充澄清建议、进化方案文档）。
*   **实施工具 (Implementation Tools)**: 不承担复杂业务逻辑，仅执行具体的原子操作（如：上传文件、创建数据集）。
