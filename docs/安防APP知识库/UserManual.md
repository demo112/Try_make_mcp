...
## 2. 安装与运行
无需安装，直接运行发布包中的 `security_kb.exe` 即可。通常需要配合 MCP Client (如 Claude Desktop, Trae 等) 使用。

### 2.1 Client 配置
请在您的 MCP Client 配置文件（例如 Claude Desktop 的配置）中添加以下内容。
**注意**：请将 `command` 中的路径替换为 `security_kb.exe` 的实际绝对路径。

```json
{
  "mcpServers": {
    "security_kb": {
      "command": "D:/path/to/security_kb.exe",
      "args": []
    }
  }
}
